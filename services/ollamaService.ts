...
export class OllamaService implements LlmService {
    ...
    private async generateContent(prompt: string): Promise<LlmResponse> {
        try {
            ...
            // The actual JSON response from the model is a string within the 'response' field
            return JSON.parse(data.response);
        } catch (error: any) {
            console.error("Error generating content with Ollama:", error);
            if (error.message.includes('Failed to fetch')) {
                 throw new Error("Failed to connect to Ollama server at http://localhost:11434. Please ensure Ollama is running locally.");
            }
            throw new Error(`Failed to generate and parse content from Ollama: ${error.message}`);
        }
    }
